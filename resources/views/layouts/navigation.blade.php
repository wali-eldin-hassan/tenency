<nav x-data="{ open: false }" class="bg-white border-b border-gray-100 dark:bg-gray-800 dark:border-gray-700">
            ...
            <div class="flex">
                ...
                <div class="flex items-center shrink-0">
                    <a href="{{ route('tenant.dashboard') }}">
                        <x-application-logo class="text-gray-500 fill-current w-9 h-9" />
                    </a>
                </div>

                <!-- Navigation Links -->
                <div class="hidden gap-x-8 sm:-my-px sm:ml-10 sm:flex">
                    ...
                    <x-nav-link :href="route('tenant.categories.index')" :active="request()->routeIs('tenant.categories.*')">
                        {{ __('Categories') }}
                    </x-nav-link>

                    <x-nav-link :href="route('tenant.customers.index')" :active="request()->routeIs('tenant.customers.*')">
                        {{ __('Audience') }}
                    </x-nav-link>
                </div>
            </div>
                ...
                @if (tenant('approved') == true)
                    <div class="flex items-center space-x-2 text-sm text-gray-600">
                        <span class="flex relative w-2.5 h-2.5">
                            ...
                            <span class="relative inline-flex w-2.5 h-.w-2.5 rounded-full bg-indigo-500"></span>
                        </span>
                ...
                @else
                    <div class="flex items-center space-x-2 text-sm text-gray-600">
                        <span class="flex relative w-2.5 h-2.5">
                            ...
                            <span class="relative inline-flex w-2.5 h-.w-2.5 rounded-full bg-red-500"></span>
                        </span>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <x-responsive-nav-link :href="route('tenant.categories.index')" :active="request()->routeIs('tenant.categories.*')">
                {{ __('Categories') }}
            </x-responsive-nav-link>

            <x-responsive-nav-link :href="route('tenant.customers.index')" :active="request()->routeIs('tenant.customers.*')">
                {{ __('Audience') }}
            </x-responsive-nav-link>
        </div>
